...
def update_callback(kernel, heartbeat):
    """ Callback to perform updates etc.
        Needs to reregister callback at end. """
    # update kernel
...
class RPCHost(AMP):
    ...
    def heartbeat_cmd(self, enabled):
        """ Enable or disable keep alive signal
            (or just pulse if no content). """
        self._heartbeat.pulse()
...
class Heartbeat:
    """ Maintains a heartbeat from client to server calls.
        Could be done on a timeout. """
